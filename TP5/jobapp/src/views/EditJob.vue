<template>
  <div class="form-container">
    <h1 class="form-title">✏️ Modifier l'emploi</h1>
    <form @submit.prevent="updateJob" class="job-form">
      <div class="form-group">
        <label>Titre :</label>
        <input v-model="editedJob.title" required class="form-input" />
      </div>
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input v-model="editedJob.experience" required class="form-input" />
        </div>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    this.editedJob = await response.json();
  },
  methods: {
    async updateJob() {
      try {
        await fetch(`http://localhost:3000/jobs/${this.id}`, {
          ...
          body: JSON.stringify(this.editedJob),
        });
